...
def summarize_jwc(data):
    items = data["items"][:20]
    # 每条带上来源（研究生院/信息学院），让 AI 能按来源平衡，也让邮件卡片能显示来源标签
    lines = "\n".join([
        "{n}. [{date}] [{source}] {title} (链接{n}: {url})".format(
            n=i + 1,
            date=x["date"],
    ...
    prompt = f"""下面是研究生院和信息学院最近发的研究生通知标题列表，方括号里是来源，每条后面标了对应的链接编号。请你严格按下面的格式输出，每一条都必须换行独占一行：

【需要行动】
来源|||标题|||链接|||一句话说明（不超过15字，说清楚要做什么/截止时间）
（最多5条，没有就不写这个分组）

【仅需了解】
来源|||标题|||链接|||一句话说明（不超过15字）
（最多5条）

格式要求：每条内容严格用 ||| 分隔来源、标题、链接、说明四部分，不要加序号，不要加"-"开头。来源必须原样照抄候选列表里方括号中的那几个字（研究生院 或 信息学院），不要自己改写。链接必须原样使用我给你的对应"链接N"的真实网址，不要编造。

来源平衡：两个来源都要覆盖，每个来源只要符合条件就至少选 1 条；同一个来源在一个分组里最多 3 条，避免被单一来源刷屏。
